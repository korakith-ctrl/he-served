...
export function cleanLogInput(form) {
  const numericFields = ["weight", "calories", "protein", "carbs", "fat", "water", "steps", "waist", "bodyFat", "muscle", "visceral", "hunger", "energy"];
...
export function upsertProfileLog(state, profileId, input) {
  ...
  const merged = { ...input, id: existing.id || `${profileId}-${input.date}`, profileId, createdAt: existing.createdAt, updatedAt: new Date().toISOString() };
  return {
    ...state,
...
const CSV_FIELDS = ["profileId", "date", "weight", "calories", "protein", "carbs", "fat", "water", "steps", "sleep", "waist", "bodyFat", "muscle", "visceral", "mood", "hunger", "energy", "workout", "restDay", "sickDay", "vacationMode", "notes"];
...
export function importCsv(text) {
  ...
  return lines.slice(1).map(line => {
    ...
    const numericFields = ["weight", "calories", "protein", "carbs", "fat", "water", "steps", "sleep", "waist", "bodyFat", "muscle", "visceral", "hunger", "energy"];
