...
import {
  ...
  Save, Scale, Sparkles, Sun, Target, TrendingDown, Trophy, Upload, Utensils, Users, Watch, Wifi, X, Zap,
} from "lucide-react";
import { z } from "zod";
...
import {
  ...
  writeRealtimePreferences, writeRealtimeWorkout,
...
function SettingsModal({open,onClose,preferences,onPreferences,syncInfo,integration,onSignIn,onSignOut,onCreatePairing,onRevokePairing}) {
  const [message,setMessage]=useState(""), [pairing,setPairing]=useState(null);
  ...
  return <Modal open={open} title="Realtime sync & reminders" onClose={onClose}>
    <div className="settings-section"><span>REMINDERS</span><p className="modal-copy">ค่าที่เลือกจะเก็บไว้ใน challenge เดียวกัน ปัจจุบันยังไม่มีระบบ push เบื้องหลัง จึงไม่อ้างว่าส่งเตือนเมื่อปิดแอป</p>{[["morningWeighIn","Morning weigh-in"],["proteinReminder","Protein reminder"],["workoutReminder","Workout reminder"],["weeklyReview","Weekly review"]].map(([key,label])=><button type="button" className="switch-row" onClick={()=>toggle(key)} key={key}><span>{label}</span><i className={preferences[key]?"on":""}><b/></i></button>)}</div>
...
export default function HealthApp() {
  ...
  const activeStore=previewDemo?demoStore:store, allLogs=activeStore.logs, profile=PROFILES[profileId], logs=allLogs[profileId], workouts=activeStore.workouts[profileId]||[], healthWorkouts=activeStore.healthWorkouts?.[profileId]||[];
  ...
  useEffect(()=>observeFirebaseAuth(user=>{setAuthUser(user);setSyncInfo(current=>({...current,user,status:user?"connecting":"signed-out",authorized:false,connected:false}))}),[]);
  useEffect(()=>{if(!authUser)return;let stop=()=>{},cancelled=false;connectRealtime(authUser,storeRef.current,{onStore:(next,member)=>{if(cancelled)return;storeRef.current=next;setStore(next);if(member?.profileId)setProfileId(member.profileId)},onState:state=>!cancelled&&setSyncInfo(current=>({...current,...state,user:authUser}))}).then(unsubscribe=>{if(cancelled)unsubscribe();else stop=unsubscribe}).catch(error=>setSyncInfo(current=>({...current,status:"error",authorized:false,connected:false,message:error.message,user:authUser})));return()=>{cancelled=true;stop()}},[authUser]);
  ...
  let content=page==="dashboard"?<Dashboard profile={profile} logs={logs} allLogs={allLogs} workouts={workouts} setPage={setPage} clock={clock}/>:page==="log"?<LogPage key={profileId} profile={profile} logs={logs} onSave={saveLog} clock={clock}/>:page==="progress"?<ProgressPage key={profileId} profile={profile} logs={logs} clock={clock}/>:page==="workout"?<WorkoutPage key={profileId} profile={profile} workouts={workouts} healthWorkouts={healthWorkouts} onSave={saveWorkout} clock={clock} preferences={store.preferences} onPreferences={updatePreferences}/>:page==="compare"?<ComparePage allLogs={allLogs} allWorkouts={activeStore.workouts} clock={clock}/>:<ProfilePage profile={profile} store={store} setPage={setPage} onReset={()=>setResetOpen(true)} onPreview={togglePreview} previewDemo={previewDemo} onImport={importFile} onSettings={()=>setSettingsOpen(true)}/>;
