...
import ClosedOrderScreen from "./components/orders/ClosedOrderScreen.jsx";
...
import { caloriesForMenu, choiceCaloriesDelta, formatCalories, menuCalories } from "./nutrition.js";
...
export default function CustomerOrder({ shopUid, eventId = null }) {
  ...
  const [closedScreenShop, setClosedScreenShop] = useState(null);
  useEffect(() => {
    if (!acceptingOrders) {
      setClosedScreenShop(shopUid);
      return;
    }
    if (closedScreenShop !== shopUid) return;
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const timer = window.setTimeout(() => setClosedScreenShop(null), reducedMotion ? 0 : 1200);
    return () => window.clearTimeout(timer);
  }, [acceptingOrders, closedScreenShop, shopUid]);
  ...
  useEffect(() => {
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const minimumDuration = reducedMotion ? 350 : (CUSTOMER_SPLASH_VARIANT === "minimal-white" ? LANDING_SCREEN_MINIMUM_MS : 2250);
  ...
  if (!acceptingOrders || closedScreenShop === shopUid) {
    return <ClosedOrderScreen shopName={shopName} logo={<BrandLogo height={72} />} opening={acceptingOrders} hasOrders={loadMyOrderIds(shopUid).length > 0} onOpenOrders={openMyOrders} />;
  }
...
function OptionPickerModal({ menu, groups, visible, onCancel, onConfirm, hideQty, initialOptions, fromTop = false, sharedTransitionName, thumbRef }) {
  ...
  const selectedCalories = caloriesForMenu(cm, selectedOptions);
  const hasCaloriesInfo = menuCalories(cm) != null || selectedOptions.some((choice) => choiceCaloriesDelta(choice) != null);
      ...
      if (initialOptions && initialOptions.length) {
        ...
        for (const o of initialOptions) {
          ...
          sel[o.groupId] = currentChoice
            ...
            : { id: o.choiceId, label: o.label, note: "", priceDelta: o.priceDelta || 0, kcalDelta: o.kcalDelta != null ? Number(o.kcalDelta) : null, ingredientId: o.ingredientId || null, qtyPercent: o.qtyPercent != null ? o.qtyPercent : 100, qtyMode:o.qtyMode || "percent", qtyValue:o.qtyValue != null ? o.qtyValue : (o.qtyPercent != null ? o.qtyPercent : 100), extraAdjustments: o.extraAdjustments || [], groupId: o.groupId, groupName: o.groupName };
        }
        setSelections(sel);
      } else {
        const defaults = {};
  ...
  function confirm() {
    ...
    const options = cg.flatMap((grp) => {
      ...
      return [{
        ...
        kcalDelta: choice.kcalDelta != null ? Number(choice.kcalDelta) : null,
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {hasCaloriesInfo && <div style={{ marginTop: 5, color: COLORS.sageDark, fontSize: 12, fontWeight: 700 }}>พลังงานโดยประมาณ {formatCalories(selectedCalories ?? menuCalories(cm))}</div>}
          </div>
        </div>
            ...
            {g.choices.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ display: "inline-flex", flexDirection: "column", alignItems: "flex-end", gap: 2, fontSize: 12.5, whiteSpace: "nowrap", marginLeft: 8 }}>
                    <span>{c.priceDelta ? `+${c.priceDelta}` : "0"}</span>
                    {choiceCaloriesDelta(c) != null && <span style={{ color: COLORS.sageDark, fontSize: 10.5, fontWeight: 700 }}>{choiceCaloriesDelta(c) > 0 ? "+" : ""}{choiceCaloriesDelta(c)} kcal</span>}
                  </span>
                </button>
              );
            })}
